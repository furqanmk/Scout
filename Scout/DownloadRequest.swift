...
fileprivate class HandlerContainer<T> {
    ...
    var success:((T) -> Void)?
    var failure:((DownloadError) -> Void)?
    init(progress: ((Double) -> Void)?, success: ((T) -> Void)?, failure: ((DownloadError) -> Void)?) {
...
public class DownloadRequest<T: DataConvertible>: NSObject, URLSessionDataDelegate {
    ...
    private lazy var session: URLSession = {
        ...
        configuration.timeoutIntervalForRequest = DownloadSettings.requestTimeoutSeconds
        configuration.timeoutIntervalForResource = DownloadSettings.requestTimeoutSeconds
        configuration.httpMaximumConnectionsPerHost = DownloadSettings.maximumSimultaneousDownloads
        configuration.requestCachePolicy = DownloadSettings.requestCachePolicy
        
        let session = URLSession(configuration: configuration, delegate: self, delegateQueue: OperationQueue.main)
        return session
    }()
    
    @discardableResult public init?(path: String, progress: ((Double) -> Void)?, success: @escaping (T) -> Void, failure: @escaping (DownloadError) -> Void) {
    ...
    public func appendHandlers(progress: ((Double) -> Void)?, success: @escaping (T) -> Void, failure: @escaping (DownloadError) -> Void) {
        let container = HandlerContainer(progress: progress, success: success, failure: failure)
        handlerContainers.append(container)
        requestCount += 1
    }
...
extension DownloadRequest {
    ...
    func signalFailure(error: DownloadError) {
        for container in handlerContainers {
            DispatchQueue.main.async {
...
extension DownloadRequest {
    static func ==(a: DownloadRequest, b: DownloadRequest) -> Bool {
        return a.url == b.url
    }
}
