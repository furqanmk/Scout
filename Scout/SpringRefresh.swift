...
public enum SpringRefreshPosition {
    case top
    case bottom
    case left
    case right
...
public extension UIScrollView {
    public func addSpringRefresh(position:SpringRefreshPosition, actionHandlere handler: @escaping (_ springRefresh:SpringRefresh) -> Void ) -> SpringRefresh {
        
        // Don't add two instance to same position.
        for v in subviews {
            if v is SpringRefresh {
                if (v as? SpringRefresh)?.position == position {
                    return v as! SpringRefresh
                }
            }
        }
        
        let springRefresh = SpringRefresh(position: position)
...
public class SpringRefresh: UIView {
    ...
    public var pullToRefreshHandler:((_ springRefresh:SpringRefresh)->Void)? = nil
    ...
    private(set) var position:SpringRefreshPosition = .top
    ...
    // - MARK: init
    public init(position:SpringRefreshPosition) {
        ...
        let springExpandView1 = SpringExpandView(frame: CGRect.zero)
        springExpandView1.isSidePosition = isSidePosition
        addSubview(springExpandView1)
        
        let springExpandView2 = SpringExpandView(frame: CGRect.zero)
        springExpandView2.isSidePosition = isSidePosition
        addSubview(springExpandView2)
        
        let springExpandView3 = SpringExpandView(frame: CGRect.zero)
